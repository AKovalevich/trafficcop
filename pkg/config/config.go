...
import (
	...
	"github.com/AKovalevich/trafficcop/pkg/entrypoint"
	log "github.com/AKovalevich/trafficcop/pkg/log/logrus"
)


const (
	// DefaultGraceTimeout controls how long trafficcop serves pending requests
...
// The main trafficcop configuration
type TrafficcopConfiguration struct {
	sync.RWMutex
	// Main configuration
	...
	// trafficcop server configuration
	ServerPort		int							`toml:"server_port" short:"sp" description:"trafficcop web server port" export:"true"`
	ServerHost		string						`toml:"server_host" short:"sd" description:"trafficcop web server host" export:"true"`
	...
	GraceTimeOut 	flaeg.Duration 				`toml:"grace_time_out" short:"g" description:"Duration to give active requests a chance to finish before trafficcop stops" export:"true"`
	// Entrypoints configuration
	EntryPoints 	entrypoint.EntrypointList 	`toml:"entry_points" short:"e" description:"trafficcop server entry points" export:"true"`
}

func NewTrafficcopConfiguration() *TrafficcopConfiguration {
	return &TrafficcopConfiguration{}
}

func NewTrafficcopDefaultConfiguration() *TrafficcopConfiguration {
	// Will take data from configuration file
	return &TrafficcopConfiguration{}
}

// Reload configuration
func (config *TrafficcopConfiguration) Reload() {
	config.Load()
}

// Load configuration from file
func (config *TrafficcopConfiguration) Load() {
	log.Do.Info("Load configuration...")
	var configFilePath string
	var tmpScreabblerConfiguration TrafficcopConfiguration
